import { Application, Assets, Sprite, Text, TextStyle, AnimatedSprite, Texture } from "pixi.js";
...
export async function initGame(container: HTMLDivElement) {
  ...
  await app.init({
    ...
    resolution: 1, // Use 1 for pixel-perfect rendering
    preference: 'webgpu', // 'webgl' or 'webgpu'
    // Plugin options
  ...
  // Set CSS to prevent image smoothing
...
function startLevel() {
  ...
  Assets.load("/sprites/hiker.json").then((atlas) => {
    ...
    // Add reverse frames for pingpong effect (12 back to 9)
    for (let i = 12; i >= 9; i--) {
      const texture = atlas.textures[`hiker ${i}.aseprite`];
      pickupFrames.push(texture);
    }
    ...
    window.addEventListener('keydown', (e) => {
      ...
      if (e.key === ' ' && !isPickingUp) {
        ...
        hiker.loop = false;
        hiker.animationSpeed = 0.15; // Much slower
        hiker.gotoAndPlay(0);
        hiker.onComplete = () => {
